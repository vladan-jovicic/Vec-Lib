...
\underline{Warning:} This  latex file do not provide (yet) any formal proof. For the moment we will just give "by hand" justifications. 
\section{Working on convex sets}
\subsection{The need of convex sets}
One important thing we have to notice is that we might have to work only on convex sets $S$. Basically if we don't it is possible that we create a polygon that have edges that are far away from the original shape. Suppose we have such a shape:
...
\draw [red] (1,0) node {$\bullet$};
\draw [red] (-1,0) node {$\bullet$};
\draw [red] (0,1) node {$\bullet$};
\draw [red] (0,-1) node {$\bullet$};
...
The green edges may one of the best approximations possible. However we cannot know given a set of point that we must do such a thing, we mean that there is a lot of particular cases that won't work. In fact we decide to say that the polygon we need is the border of the convex hull of the return set of point. Then we need to work on convex sets. Moreover one can notice that the green solution is not a polygon. 

\subsection{How to get convex sets}
First of all let us show how to detect a certificate of non-convexity.
...
\foreach \xp in {.5,1}{
\draw [-] (3,\xp) -- (3,\xp+.5);
\draw [-] (3.5,\xp) -- (3.5,\xp+.5);
\draw [-] (3,\xp+.5) -- (3.5,\xp+.5);
}
\node at (3.25,2) {$\vdots$};
\draw [-] (3,2.5) -- (3.5,2.5);
\foreach \xp in {2.5,3}{
...
\draw [<->] (0,-.5) -- node [below] {$n$ pixels} (3.5,-.5);
\draw [<->] (4,0) -- node [right] {$m$ pixels} (4,3.5);

\node at (6,-.5) {The interior of the set};
\end{tikzpicture}
\end{center}
We may show that at least one of this integer $n$ or $m$ equals $1$.
...
One can notice that in the constraint $\forall e\in S, \sum_{f,f'\in S}a_{e,f,f'}x_fx_{f'}>0$, since all $a_{e,f,f'}$ are non negative, we just need to know if one is positive and if the corresponding $z_{f,f'}$ is $1$. Let us now introduce: 
\[\tilde{a}_{e,f,f'}=\left\{\begin{array}{r l}
1 & \text{if $a_{e,f,f'}>0$}\\
0 & \text{otherwise}
\end{array}\right.\]

We may now solve the following equivalent problem:
\[\begin{array}{r l}
&\min\sum_{e\in S} x_e\\
\text{respect to}&\forall e\in S, \sum_{f,f'\in S}\tilde{a}_{e,f,f'}x_fx_{f'}\geq 1\\
&\forall e\in S, x_e\in\{0,1\}
\end{array}\]
Notice that we put $\geq 1$ instead of $>0$ since we now have integers.
...
This program returns a set of point that minimizes the number of points and such that the maximum distance between an element of $S$ and the convex hull of the returned set is at most $\delta$. Then if $S$ represents a convex set, it will be approximated by a polygon which will be included in and the that the border have maximum distance $\delta$. 
