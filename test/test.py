...
class Test:
	...
	def run_harris_corner_detector(self, disp_original_cont, disp_corners, output):
		points, sep_idx, separators = self.read_points()
		print("Num of contours: %d" % separators)
		input_pols = []
		segments, all_corners, all_curves, all_pols = [], [], [], []
		for i in range(separators):
			print("Working on contour %d" % i)
			curr_points = points[sep_idx[i]:sep_idx[i + 1]]
		...
		if disp_corners:
			...
			plt.plot(x_pts, y_pts, 'ro', marker='*')
			plt.show(block=True)
